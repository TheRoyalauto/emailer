    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-[#E5E7EB] dark:border-slate-700 max-w-2xl w-full shadow-2xl overflow-hidden">
                {/* Header */}
                <div className="px-6 py-4 border-b border-[#E5E7EB] dark:border-slate-700 bg-[#f8fafc] dark:bg-slate-800/50">
                    <h2 className="text-lg font-bold text-[#0f172a] dark:text-white">Merge {contacts.length} Contacts</h2>
                ...
                <div className="p-6 space-y-4">
                            ...
                            {contacts.map(c => (
                                <button
                                    ...
                                    className={`w-full text-left p-3 rounded-xl border transition-all ${primaryId === c._id
                                        ? "border-[#0EA5E9] bg-[#0EA5E9]/5 dark:bg-[#0EA5E9]/10 ring-1 ring-[#0EA5E9]/30"
                                        : "border-[#E5E7EB] dark:border-slate-700 hover:border-[#9CA3AF] dark:hover:border-slate-500"
                                        }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${primaryId === c._id
                                            ? "bg-[#0EA5E9] text-white"
                                            : "bg-[#f8fafc] dark:bg-slate-800 text-[#9CA3AF]"
                                            }`}>
                                            {c.name?.charAt(0).toUpperCase() || c.email.charAt(0).toUpperCase()}
                                        </div>
                                        <div>
                                            <div className="text-sm font-medium text-[#0f172a] dark:text-white">{c.name || "—"}</div>
                    ...
                    <div>
                        ...
                        <div className="p-4 bg-[#f8fafc] dark:bg-slate-800 rounded-xl border border-[#E5E7EB] dark:border-slate-700 space-y-2">
                            <div className="grid grid-cols-2 gap-2 text-sm">
                                <div>
                                    <span className="text-[#9CA3AF]">Name:</span>{" "}
                                    <span className="text-[#0f172a] dark:text-white font-medium">{mergedPreview.name || "—"}</span>
                                </div>
                                <div>
                                    <span className="text-[#9CA3AF]">Email:</span>{" "}
                                    <span className="text-[#0f172a] dark:text-white font-medium">{mergedPreview.email}</span>
                                </div>
                                <div>
                                    <span className="text-[#9CA3AF]">Company:</span>{" "}
                                    <span className="text-[#0f172a] dark:text-white font-medium">{mergedPreview.company || "—"}</span>
                                </div>
                                <div>
                                    <span className="text-[#9CA3AF]">Phone:</span>{" "}
                                    <span className="text-[#0f172a] dark:text-white font-medium">{mergedPreview.phone || "—"}</span>
                                </div>
                            </div>
                    ...
                    <div className="bg-[#FEF2F2] dark:bg-red-950/30 border border-[#EF4444]/20 rounded-xl p-3">
                ...
                {/* Footer */}
                <div className="px-6 py-4 border-t border-[#E5E7EB] dark:border-slate-700 bg-[#f8fafc] dark:bg-slate-800/50 flex justify-between">
                    <button
                        onClick={onClose}
                        className="px-4 py-2.5 text-sm text-[#4B5563] dark:text-slate-400 hover:text-[#0f172a] dark:hover:text-white transition-colors"
                    >
                        Cancel
                    </button>
                    <button
